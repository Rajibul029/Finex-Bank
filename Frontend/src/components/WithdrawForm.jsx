...
import { withdrawMoney } from "../api/accountApi";
...
export default function WithdrawForm() {
  ...
  const handleWithdraw = async (e) => {
    ...
    setMsg("");
    try {
      const res = await withdrawMoney(parseFloat(amount));
      setMsg(res.data.message || "Withdrawal successful!");
      setAmount("");
    } catch (err) {
      setMsg(err.response?.data?.detail || "Withdrawal failed!");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-400 opacity-10 blur-3xl rounded-full"></div>
